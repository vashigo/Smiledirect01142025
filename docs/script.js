...
// Renderizar soundtracks
function renderSoundtracks(soundtracks) {
  const container = document.getElementById('soundtrack-cards');
  soundtracks.forEach(st => {
    ...
    format.textContent = `Formato: ${st.format}`;

    const region = document.createElement('p');
    region.textContent = `Región: ${st.region}`;
...
async function init() {
  try {
    ...
    renderSoundtracks(data.soundtracks);
